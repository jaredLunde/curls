...
test('<Button> -> themed scale', () => {
  ...
  const sizes = ['sm', 'md', 'lg']
  for (let size of sizes)
    expect(renderFragment(<Button size={size}/>, theme)).toMatchSnapshot(`css.${size}`)
  ...
  for (let size of sizes)
    expect(renderFragment(<Button size={size}/>, theme)).toMatchSnapshot(`fn.${size}`)
})

test('<Button> -> active/hover class', () => {
  const theme = {
    button: {
      getHoverClass: () => css`&:hover { transform: scale(2.0); }`,
      getActiveClass: () => css`&:active { transform: scale(1.0); }`
    }
  }

  expect(renderFragment(<Button/>, theme)).toMatchSnapshot('2.0@hover, 1.0@active')
})